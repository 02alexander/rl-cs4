...
impl<G> Evaluator<G> for CNNEval
where
    G: Game,
{
...
impl<'de> Visitor<'de> for ByteDataVisitor {
        ...
        S: SeqAccess<'de>,
    {
        ...
        let model = TrainableCModule::load(&fname, vs.root())
            .expect(&format!("couldn't load module from file {}", &fname));
        std::fs::remove_file(&fname).unwrap();
        Ok(CNNEval { model, vs })
    }
}
